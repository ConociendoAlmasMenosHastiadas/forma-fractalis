...
pub enum Commands {
    ...
    /// Validate GPU shader correctness by comparing GPU and CPU rendered outputs
    GpuTest {
        /// Test only this fractal (optional; tests all GPU-supported fractals if omitted)
        #[arg(long)]
        fractal: Option<String>,

        /// Render width for test images (default: 512)
        #[arg(long, default_value = "512")]
        width: u32,

        /// Render height for test images (default: 512)
        #[arg(long, default_value = "512")]
        height: u32,
...
pub fn render_from_cli(args: &Commands) -> Result<(), String> {
    match args {
        Commands::GpuTest { fractal, width, height, iterations } => {
            #[cfg(feature = "gpu")]
            {
                let config = crate::gpu_test::GpuTestConfig {
                    width: *width,
                    height: *height,
                    iterations: *iterations,
                    fractal_name: fractal.clone(),
                    output_dir: std::path::PathBuf::from("temp/gpu_test"),
                };
                let results = crate::gpu_test::run_gpu_tests(&config)?;
                let failed: Vec<_> = results.iter().filter(|r| !r.passed()).collect();
                if failed.is_empty() {
                    Ok(())
                } else {
                    Err(format!("{} GPU test(s) failed", failed.len()))
                }
            }
            #[cfg(not(feature = "gpu"))]
            {
                let _ = (fractal, width, height, iterations);
                Err("GPU support is not compiled in (build with --features gpu)".to_string())
            }
        }
        Commands::Render {
            input,
