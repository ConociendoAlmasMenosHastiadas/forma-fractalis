...
fn benchmark_fractal(
    ...
    for _ in 0..2 {
        ...
        #[cfg(not(feature = "gpu"))]
        let _ = render_with_config(&config, RenderTarget::Preview);
    }
...
fn main() {
    ...
    println!("\n--- FractalConfig API benchmark (1280x720, 256 iter) ---");
    for (name, fractal) in &fractals {
        ...
        for _ in 0..5 {
            ...
            let _ = render_fractal_to_buffer(fractal.as_ref(), &config);
            times.push(start.elapsed().as_secs_f64() * 1000.0);
        }
        let avg = times.iter().sum::<f64>() / times.len() as f64;
        println!("{:<20} via FractalConfig API: avg={:.2}ms", name, avg);
    }
