...
/// Export a fractal image using the high-level config types.
///
/// This is the `FractalConfig`-based equivalent of `export_png`. Library users
/// who build a `FractalConfig` can call this instead of the low-level function.
///
/// `export_config` controls scale, supersampling, and filtering.
/// `backend` / `hiprec_bits` / `max_threads` control the rendering backend.
#[cfg(feature = "gpu")]
pub fn export_png_with_config(
    ...
    export_png(
        &config.view,
        &config.color_config.colormap,
        config.max_iterations,
        fractal,
        &config.fractal_parameters,
        config.color_config.use_period,
        config.color_config.period,
...
/// Export a fractal image using the high-level config types (non-GPU build).
#[cfg(not(feature = "gpu"))]
pub fn export_png_with_config(
    fractal: &dyn Fractal,
    config: &crate::config::FractalConfig,
...
mod tests {
    ...
    fn test_metadata_struct() {
        ...
        assert_eq!(metadata.parse_fractal_type().unwrap(), crate::fractals::FractalType::Julia);
    ...
    fn test_metadata_parse_fractal_types() {
        let test_cases = vec![
            ("Mandelbrot", crate::fractals::FractalType::Mandelbrot),
            ("Julia", crate::fractals::FractalType::Julia),
...
impl FractalMetadata {
    ...
    pub fn parse_fractal_type(&self) -> Result<crate::fractals::FractalType, String> {
        use crate::fractals::FractalType;
        ...
        match self.fractal_type.as_str() {
            ...
            "Tetration" => Ok(FractalType::Tetration),
            "Lemon" => Ok(FractalType::Lemon),
